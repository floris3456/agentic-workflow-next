...
runtime-owned HOME/config/cache/temp, persistent data/state, one explicit provider credential, project config/
...
executable version/OpenAPI, and the exact read-only Scout agent/tool contract.
...
The bridge-owned read-only Scout prompt treats all repository instructions as
...
accepts only the public `small|heavy` selector (default `small`), mapped to
`small-workspace-maintainer` and `workspace-maintainer`; route changes, bridge
PTYs, and main promotion are denied for workspace tasks.
...
provider credential. It drives the production bridge start, continuation,
