...
The hidden `agentic-bridge/1` command envelope carries a UUID, task ID, monotonic sequence, kind, arguments, and optional expected Git state. The first issue command must be `start` at sequence `1`; later accepted sequences are exactly contiguous. SQLite records accepted/applying/terminal states before and after side effects, and a task cannot admit another command while one is accepted or applying. Admission rejections are durable without consuming sequence. Duplicate UUIDs do not reapply. The bridge queues and attempts public `applying` publication before the handler starts. A restart while applying marks the command indeterminate and never guesses or retries the side effect.

Sequence-free `agentic-bridge-request` envelopes provide task-bound durable recovery views. `command.status` reads exact command or pre-ledger-rejection state, known projected result, timing, applying age, and service heartbeat. `task.status` reads mapped session state and the latest projected developer response. They have a separate UUID ledger, do not consume command sequence, and never call a mutation handler. A genuinely stuck applying state receives one bounded reconciliation; uncertain mutation is never retried, and operator restart converts it to indeterminate before evidence-based recovery.
...
Mapped developer `session.idle` and `session.error` events durably queue delivery of the latest assistant message. The bridge applies the existing public projection, stores the task-correlated projected response, and queues it to the bound issue; failures remain retryable. It performs only structural latest-message selection and transport. The web orchestrator, not the bridge, interprets response status and decides whether direct remote review can begin.
