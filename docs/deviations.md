...
- Selected alternative: `template-development` remains independent, is never
  ...
  template-development range excludes all package storage paths beneath
  `changes/**` so package storage is never transferred inside another package.
  Superseding packages coexist in distinct revision directories (e.g.
  `changes/<task-id>.rev2/`) referencing the historical package's exact SHA-256
