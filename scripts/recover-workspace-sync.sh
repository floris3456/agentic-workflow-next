...
cd "$root"
[[ "$(git branch --show-current)" == "workspace" ]] || { echo "Recovery requires workspace." >&2; exit 1; }
[[ -z "$(git status --porcelain)" ]] || { echo "Working tree must be clean before recovery." >&2; exit 1; }
./scripts/bootstrap-workspace.sh --check >/dev/null

git_dir="$(git rev-parse --git-dir)"
marker="$git_dir/workspace-sync-failed"
authorization="$git_dir/workspace-sync-recovery-authorized"
[[ -f "$marker" ]] || { echo "Recovery requires the failed-push marker." >&2; exit 1; }
mapfile -t fields < "$marker"
[[ "${#fields[@]}" -eq 3 && "${fields[0]}" == "workspace" && "${fields[1]}" =~ ^[0-9a-f]{40}$ ]] || {
...
trap cleanup EXIT
git fetch --no-tags origin workspace
local_sha="$(git rev-parse HEAD)"
remote_sha="$(git rev-parse origin/workspace)"

verify() {
  git fetch --no-tags origin workspace
  local verified_local verified_remote
  verified_local="$(git rev-parse HEAD)"
  verified_remote="$(git rev-parse origin/workspace)"
  ...
  rm -f "$marker"
  echo "Workspace synchronization recovered at $verified_remote"
}
...
if git merge-base --is-ancestor "$local_sha" "$remote_sha" && git merge-base --is-ancestor "$failed" "$remote_sha"; then
  git merge --ff-only origin/workspace
  verify
  exit 0
fi
...
if ! git merge --no-ff --no-edit -m "Recover workspace synchronization" "$remote_sha"; then
