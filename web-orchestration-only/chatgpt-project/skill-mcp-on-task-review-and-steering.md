...
8. Require `Changed approach` to describe old/new route, steering source, and treatment of prior work.
9. Create a new task only when the intended outcome materially changes. Finish the old task and close its control issue before opening another.
10. Prefer corrective commits or `git revert`; avoid shared-history rewriting.
11. Do not count external blockers or brief, GitHub, or bridge transport defects as Luna failures.
12. If synchronization breaks, stop implementation and use synchronization recovery.
13. Persist review conclusions, issue reference, command ID, sequence, and consequential steering in task context.
