...
5. Add these seven rendered files as individual Project Sources, retaining each
   exact filename:
...
6. Configure connected/native GitHub access for exact repository reads and comparisons; issue list/search, create/read/update/comment/close/reopen, label read/write, and comment-author metadata; and narrowly scoped writes under `web-orchestration-only/task-context/**` on `web-orchestration`.
...
3. Replace the Project developer instructions and install the exact seven-Source
