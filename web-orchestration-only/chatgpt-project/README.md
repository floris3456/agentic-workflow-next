...
5. Add these twelve rendered files as individual Project Sources, retaining each
   exact filename:
...
8. Do not configure a direct OpenCode tool in the Project. MCP-ON launches the
   ...
   GitHub Issues when delegation is selected; MCP-OFF has neither capability.
   Prompt creation may produce text for a separate direct OpenCode session, but
   generating that text does not configure or invoke OpenCode from the Project.
...
plus the cross-mode prompt-creation core. Its destination, mission, and craft
profile Sources are support Sources loaded only by that core; they are
intentionally not standalone router entries. Shared authority and public-safety
boundaries stay permanently visible in the router.
...
3. Replace the Project developer instructions and install the exact twelve-Source
