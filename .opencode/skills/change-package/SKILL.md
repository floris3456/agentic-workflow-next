...
- Keep source implementation on its canonical branch. `workspace` owns only its maintenance runtime, ledger state, and package machinery; never copy or merge independent source trees into it.
...
- Generate a package only with `scripts/create-change-package.mjs` over exact reviewed `workspace`, `developer`, and `orchestration` ranges. Do not hand-build package bytes or weaken provenance checks.
- Package storage under `changes/**` is ledger-only and excluded from portable workspace patch content. A superseding package uses a distinct revision directory and preserves earlier evidence.
